# 🌿 Boulder Green Commute Guide

> A community resource helping Boulder, Colorado residents make sustainable travel choices — one step and one pedal at a time.

---

## 🚶 Walking & 🚴 Biking Commute Analysis

This guide presents a detailed analysis of walking and biking commute options for a regular route in Boulder, CO, using live OpenStreetMap routing data:

| Location | Address | Coordinates |
|----------|---------|-------------|
| 🏠 **Home** | 45 Oak Avenue, Boulder, CO 80304 | 40.0448805°N, 105.2738879°W |
| 🏢 **Office** | 1800 Broadway, Boulder, CO 80302 | 40.0163281°N, 105.2789726°W |

### At a Glance

| Metric | 🚶 Walking | 🚴 Biking |
|--------|-----------|----------|
| Distance | 4.03 km / 2.5 miles | 4.03 km / 2.5 miles |
| Est. Time | ~48–50 min | ~12–15 min |
| Avg Speed | 5 km/h (3.1 mph) | 16 km/h (10 mph) |
| Calories Burned | ~280–340 kcal | ~120–180 kcal |
| CO₂ Saved vs. Car | ~0.9 kg/trip | ~0.9 kg/trip |
| Trips/yr CO₂ Saved | ~234 kg (260 round trips) | ~234 kg |
| Cost | Free | Free |
| Weather Dependent | Yes — Bundle up in winter! | Yes — Use aLayers! |

---

## 🗺️ Route Overview

Both walking and biking routes follow the same scenic corridor through central Boulder, passing near the CU Boulder campus and downtown:

```
Oak Avenue → 19th Street → 20th Street → Pearl Street → 18th Street → Canyon Boulevard
```

### Step-by-Step Walking & Biking Directions

| Step | Street | Distance | Walk Time | Bike Time | Description |
|------|--------|----------|-----------|-----------|-------------|
| 1 | Oak Avenue | 97 m | ~12 s | ~14 s | Start on quiet residential Oak Ave heading south |
| 2 | 19th Street | 2,157 m | ~4 min | ~1 min 20 s | Broad, straight stretch southbound — most of the route |
| 3 | 20th St. | 106 m | ~13 s | ~15 s | Brief intersection transition |
| 4 | 20th Street | 566 m | ~7 min | ~2 min | Continue south through residential & campus area |
| 5 | Pearl Street | 216 m | ~28 s | ~17 s | Cross vibrant Pearl Street Mall — downtown heart |
| 6 | 18th Street | 249 m | ~31 s | ~18 s | South on 18th toward Canyon |
| 7 | Canyon Boulevard | 637 m | ~1 min | ~28 s | Scenic final stretch arriving via Canyon Blvd 🏁 |
|  | **TOTAL** | **4,027 m / 2.5 mi** | **~48–50 min** | **~12–15 min** |  |

---

## 🌍 Why Green Commuting Matters

Boulder is a city that leads by example in sustainability. By choosing to walk or bike for your daily commute, you're:

- 🌱 **Reducing emissions** — A single 4 km car trip produces ~0.9 kg of CO₂. Switching to walking or biking eliminates this entirely.
- 💪 **Improving health** — Walking 30+ min/day reduces heart disease risk by up to 30%, improves mood, and boosts energy.
- 💰 **Saving money** — The average American spends $0.65/mile driving; walking/biking costs $0 per mile.
- 🚗 **Reducing congestion** — Fewer cars means better traffic for everyone, especially during flood season on Walnut.
- 🦎 **Supporting Boulder's green ethos** — Boulder has 300+ miles of bike paths and 45,000+ acres of open space.

---

## 🗂️ Repository Files & Resources

| File | Description |
|------|-------------|
| `routes/commute-analysis.md` | 📝 Detailed walk & bike commute analysis |
| `routes/route.geojson` | 🗺️ Interactive route geometry for mapping tools |
| `routes/bike-strategy.md` | 🚴 Boulder bike-specific strategy & infrastructure guide |

---

## 🚴 Boulder Biking Resources

- [Boulder B-Cycle](https://bouldercolorado.gov/bcycle) — Bike share program, 44 stations
- [Boulder Connectivity Map](https://bouldercolorado.gov/streets/connectivity) — Official bike network
- [RTD Bike & Ride](https://www.rtd-denver.com/riding/bike-and-ride) — Combine biking with public transit
- [CU B-cycle](https://www.cubicycle.org/) — University bike-share, convenient stations near this route
- [OpenStreetMap Boulder](https://www.openstreetmap.org/#map=15/40.0163/-105.2790) — Explore the route live

---

## 🌎 Route on OpenStreetMap

Explore this commute route live on OpenStreetMap:

- **Home (45 Oak Ave):** [View on OSM](https://www.openstreetmap.org/#map=15/40.0449/-105.2739)
- **Office (1800 Broadway):** [View on OSM](https://www.openstreetmap.org/#map=15/40.0163/-105.2790)
- **Route map (OpenStreetMap 编辑器):** 

---

## 🤝 Contributing

This is a community guide! If you have suggestions for improving walking or biking routes, adding new commute analyses, or correcting data, please open an issue or submit a pull request.

### How to Add Your Own Commute Analysis

1. **Geocode** your home and work addresses using [Nominatim](https://nominatim.openstreetmap.org/)
2. **Run** a commute analysis using the [OSRM live demo](https://map.project-osrm.org/) or [OSM Tools API](https://tools.interlier.com/)
3. **Add** your route details as a new `.md` file under `/routes`
4. **Include** a GeoJSON file of the route geometry if available
5. **Submit** a pull request!

---

## 📊 Data Sources

- **Geocoding & Routing:** [OpenStreetMap (OSM)](https://www.openstreetmap.org/) & [Nominatim API](https://nominatim.openstreetmap.org/)
- **Route Directions:** OSRM (Open Source Routing Machine)
- **Emission Factors:** EPA Green Vehicle Calculator, GHG Emission Factors (2024)
- **Calorie Estimates:** Compendium of Physical Activities (Ainsworth et al., 2011)
- **Boulder bike infrastructure:** City of Boulder Mobility & Connectivity Plan

---

## 📬 Get Involved

- 🔧 Open an [Issue](https://github.com/zhub9006/boulder-green-commute-guide/issues) to suggest improvements
- 📧 Share this guide with fellow Boulder residents looking to abandon the I-14 shoulder traffic
- 🚲 Advocate for more protected bike lanes in Boulder!
- 🌎 Start a similar repo in your city — open-source sustainability!

---

*Made with ❤️ for a greener Boulder. Live data updated August 2026. Data © OpenStreetMap contributors, ODbL 1.0.*
